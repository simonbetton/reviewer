          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("SB Code Review (Alpha)"),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          userDataPath,
          "/Users/alice/Library/Application Support/SB Code Review (Alpha)",
        );
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/SB Code Review (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["SB Code Review (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "SB Code Review (Alpha)");
